...
// clang cannot deduce the return type -> added TReturn and replaced decltype(auto)
template<typename TReturn, typename TVisitor, typename TVariant>
TReturn visit_node_recursively(TVisitor && visitor, TVariant && variant)
{
    return std::visit(
    ...
    auto make_recursive_visitor(TFs &&... fs)
    {
        return boost::hana::fix([&fs...](auto self, auto && x) -> TReturn {
            return boost::hana::overload(std::forward<TFs>(fs)...)(
                [&self](auto && v) { return visit_node_recursively<TReturn>(self, v); },
...
int main()
{
    auto lambda_visitor = make_recursive_visitor<double>
    (
    [](auto, auto x) {  x.f(); },
    [](auto, unsigned int x) { return static_cast<double>(x); },
    [](auto, int x) { return static_cast<double>(x); },
    [](auto, float x) { return static_cast<double>(x); },
    [](auto, double x) { return x; },
    [](auto visit_self, onode const & pn) {
            auto & node = *pn;
            switch (node.op) {
            case ub_operator::uminus:
                assert(node.nodes.size() == 1);
                return -visit_self(node.nodes[0]);
            case ub_operator::add:
                assert(node.nodes.size() == 2);
                return visit_self(node.nodes[0]) +
                       visit_self(node.nodes[1]);
            case ub_operator::subtract:
                assert(node.nodes.size() == 2);
                return visit_self(node.nodes[0]) -
                       visit_self(node.nodes[1]);
    ...
    while (std::getline(std::cin, str)) {
        ...
        if (r) {
            ...
            std::cout << str << " ==> "
            << std::visit(lambda_visitor, client::vn_stack[0])
            << " (AST eval)"
            << std::endl;
